...
class CoefficientMapping(torch.nn.Module):
        ...
        for l in range(lmax + 1):
            for m in range(lmax + 1):
                ...
                self.register_buffer(
                    f"coefficient_idx_l{l:d}_m{m:d}", 
                    mask_indices, 
                    persistent=False,
                )
    ...
    def prepare_coefficient_idx(self):
        ...
        for l in range(lmax + 1):
            l_list = []
            for m in range(lmax + 1):
                l_list.append(getattr(self, f"coefficient_idx_l{l:d}_m{m:d}", None))
    ...
    def pre_compute_rotate_inv_rescale(self):
        ...
        for l in range(lmax + 1):
            for m in range(lmax + 1):
                ...
                self.register_buffer(
                    f"rotate_inv_rescale_l{l:d}_m{m:d}",
                    rotate_inv_rescale,
                    persistent=False,
                )

    def __repr__(self):
        return f"{self.__class__.__name__}(lmax={self.lmax:d}, mmax={self.mmax:d})"
