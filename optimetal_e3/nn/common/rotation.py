...
class Safeacos(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x):
        x_clamped = x.clamp(-1.0+EPS, 1.0-EPS)
...
def init_edge_rot_euler_angles(edge_vector, check_equivariance):
    # we need to clamp the output here because if using compile
    # normalize can return >1.0 , pytorch #163082
    xyz = torch.nn.functional.normalize(edge_vector).clamp(-1.0, 1.0)
