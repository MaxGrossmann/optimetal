...
from optimetal_e3.nn.common.activation import GateActivation
...
from optimetal_e3.nn.common.so2_layers import SO2_Convolution
from optimetal_e3.nn.common.so3_layers import SO3_Linear
...
class Edgewise(torch.nn.Module):
    def __init__(
        ...
        self.edge_channel_list = copy.deepcopy(edge_channel_list)
        ...
        self.so2_conv_2 = SO2_Convolution(
            ...
            edge_channel_list=None,
            extra_m0_output_channels=None,
        )

        self.cutoff = cutoff

        self.out_mask = self.SO3_grid["lmax_lmax"].mapping.coefficient_idx(self.lmax, self.mmax)
...
class SpectralAtomwise(torch.nn.Module):
    def __init__(
        ...
        # ignore no-op warning when 'lmax' is 0
...
class eSCNMD_Block(torch.nn.Module):
    def __init__(
        ...
        self.norm_1 = EquivariantRMSNormArraySphericalHarmonics(
            lmax=self.lmax,
        ...
        self.edge_wise = Edgewise(
            ...
            lmax=lmax,
            mmax=mmax,
            edge_channel_list=edge_channel_list,
            mappingReduced=mappingReduced,
            SO3_grid=SO3_grid,
            cutoff=cutoff,
        )

        self.norm_2 = EquivariantRMSNormArraySphericalHarmonics(
            lmax=self.lmax,
            num_channels=sphere_channels,
        )
