...
import optimetal_e3.utils as utils
...
class EnsembleEvaluator:
    ...
    def __init__(
        ...
        for best_model_path in self.best_model_paths:
            ...
            self.amp_dtypes.append(best_model_dict["precision"])
            self.model_kwargs.append(best_model_dict["model_kwargs"])
            self.eps_weights.append(best_model_dict["eps_weight"])
            self.drude_weights.append(best_model_dict["drude_weight"])
            self.seeds.append(best_model_dict["seed"])
            
        # check that all models match
        if not all(kwargs == self.model_kwargs[0] for kwargs in self.model_kwargs):
            raise ValueError("Not all 'model_kwargs' are identical across models.")
        self.model_kwargs = self.model_kwargs[0]
        
        # check that all loss weights match
        if not all(eps_weight == self.eps_weights[0] for eps_weight in self.eps_weights):
            raise ValueError("Not all 'eps_weight' are identical across models.")
        self.eps_weight = self.eps_weights[0]
        if not all(drude_weight == self.drude_weights[0] for drude_weight in self.drude_weights):
            raise ValueError("Not all 'drude_weight' are identical across models.")
        self.drude_weight = self.drude_weights[0]
        
        # check that all seeds are different (otherwise, we would not have an ensemble of models trained with different random seeds...)
        if not all(seed != self.seeds[0] for seed in self.seeds[1:]):
            raise ValueError("All 'seed' must be different across models.")
        self.seed = self.seeds[0] # we just pick the first seed for reproducibility in the sampling of materials for plotting, etc.
        ...
        self.loss_fn_eps = torch.nn.L1Loss(reduction="mean")
        self.loss_fn_drude = torch.nn.L1Loss(reduction="mean")
    ...
    def _load_models(self) -> None:
        ...
        for i, best_model_dict in enumerate(self.best_model_dicts):
            model = OptiMetalE3(
                lmax=self.model_kwargs["lmax"],
                mmax=self.model_kwargs["mmax"],
                hidden_channels=self.model_kwargs["hidden_channels"],
                ff_type=self.model_kwargs["ff_type"],
            )
    ...
    def quantile_plot(
        ...
        if rng_seed is None:
            rng = np.random.default_rng(self.seed) # use the first seed for reproducibility if no seed is provided
        else:
            rng = np.random.default_rng(rng_seed)
        quartile_idxs = []
