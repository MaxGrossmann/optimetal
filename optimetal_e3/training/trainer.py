...
from optimetal_e3.utils import listdir_full
...
class Trainer:
    ...
    def __init__(
        ...
        The 'trainer_dict' dictionary contains:
            ...
            "device":           torch device 
            "train_loader":     Dataloader for the training data (must be a 'torch_geometric.loader.DataLoader' instances)
            "val_loader":       Dataloader for the validation data (must be a 'torch_geometric.loader.DataLoader' instances)
            "model":            Instance of the model to be trained (must be a 'torch.nn.Module' instance)
            "model_kwargs":     (Optional) Dictionary of keyword arguments that were used to initialize the model
            "lr":               Learning rate for the AdamW optimizer
            "weight_decay":     Weight decay for the AdamW optimizer
            "T_max":            Maximum number of epochs in a cycle for the cosine annealing learning rate scheduler
        ...
        self.model_kwargs = trainer_dict.get("model_kwargs", {})
        if not isinstance(self.device, torch.device):
            raise ValueError("The 'device' must be an instance of 'torch.device'")
        if not isinstance(self.train_loader, DataLoader):
            raise ValueError("The 'train_loader' must be an instance of 'torch_geometric.loader.DataLoader'")
        if not isinstance(self.val_loader, DataLoader):
            raise ValueError("The 'val_loader' must be an instance of 'torch_geometric.loader.DataLoader'")
        if not isinstance(self.model, torch.nn.Module):
            raise ValueError("The 'model' must be an instance of 'torch.nn.Module'")

        # training hyperparameters
        self.lr = trainer_dict["lr"]
        self.weight_decay = trainer_dict["weight_decay"]
        self.T_max = trainer_dict["T_max"]
        ...
        # fixed loss function, optimizer, and learning rate scheduler
    ...
    def _train_step(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        for batch in self.train_loader:
            ...
            with torch.autocast(
                ...
                # 'eps'   - (batch_size * 2001, 2), 1st column is the real part, 2nd column is the imaginary part
                # 'drude' - (batch_size) 
                eps, drude = self.model(batch)
                # loss functions
                # 'batch.eps'   - (batch_size * 2001, 2), 1st column is the real part, 2nd column is the imaginary part
                # 'batch.drude' - (batch_size) 
    ...
    def _save_model(self) -> None:
        ...
        checkpoint = {
            ...
            "model_kwargs": self.model_kwargs, # good to know
            "eps_weight": self.eps_weight, # good to know
            "drude_weight": self.drude_weight, # good to know
            "seed": self.seed, # good to know
        }
