...
def parse_cse_entry(file_path: str) -> ComputedStructureEntry:
    ...
    if not os.path.exists(file_path):
        raise FileNotFoundError(f"The specified JSON file '{file_path:s}' does not exist")
...
def parallel_parse_cse_entries(file_path_list: list[str]) -> list[ComputedStructureEntry]:
    ...
    for path in file_path_list:
        if not os.path.exists(path):
            raise FileNotFoundError(f"One of the specified JSON files '{path:s}' does not exist")
...
def parse_h5_file(file_path: str) -> list[dict]:
    ...
    """
    if not os.path.exists(file_path):
        raise FileNotFoundError(f"The specified H5 file '{file_path:s}' does not exist")
...
def load_torch_data(data_path: str) -> dict:
    ...
    Input:
        data_path:          Path to a file containing torch data
    Output:
    ...
    if not os.path.exists(data_path):
        raise FileNotFoundError(f"The specified path '{data_path:s}' does not exist")
